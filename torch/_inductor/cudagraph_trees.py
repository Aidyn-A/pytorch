...
# device -> {pool_id: (MemPool, stream)}; keeps the MemPool alive while registered.
...
def _resolve_device(device: int | torch.device | None) -> int:
    if device is None:
...
def register_external(
    ...
    """Register ``pool`` so cudagraph_trees tracks/checkpoints it alongside the private pool."""
...
def unregister_external(
    ...
    """Remove a previously registered pool."""
    dev = _resolve_device(device)
...
def check_memory_pool(
    ...
    # Fast path: single pool can use the cheap native check
    ...
    # Only the private pool (index 0) flags untracked blocks as leaks;
    # external pools are shared and may hold non-cudagraph allocations.
...
class CUDAGraphTreeManager:
    ...
    def apply_checkpoint_execution_state_in_allocator(self) -> None:
        ...
        # Verify pool state matches tracked live storages
        if config.triton.slow_path_cudagraph_asserts:
            check_memory_pool(
                self.device_index,
